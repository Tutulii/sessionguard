export const productionSchemaSql = `
...
CREATE TABLE IF NOT EXISTS users (
  ...
  chain_id integer NOT NULL CHECK (chain_id = 42161),
...
CREATE TABLE IF NOT EXISTS market_anchors (
  ...
  quality text NOT NULL CHECK (quality IN ('OBSERVED', 'DEGRADED')),
...
CREATE TABLE IF NOT EXISTS decisions_v1 (
  ...
  created_at timestamptz NOT NULL,
  symbol text NOT NULL,
  permission text NOT NULL CHECK (permission IN ('TRADE', 'ALERT_ONLY', 'BLOCK')),
...
CREATE TABLE IF NOT EXISTS paper_orders (
  id uuid PRIMARY KEY,
  decision_id uuid NOT NULL UNIQUE REFERENCES decisions_v1(id) ON DELETE CASCADE,
  user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS notification_channels (
  id uuid PRIMARY KEY,
  user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  type text NOT NULL CHECK (type IN ('IN_APP', 'TELEGRAM', 'EMAIL', 'WEB_PUSH')),
  label text NOT NULL,
...
CREATE TRIGGER audit_events_immutable BEFORE UPDATE OR DELETE ON audit_events
  FOR EACH ROW EXECUTE FUNCTION sessionguard_protect_audit_events();
`;

export const productionSchemaVersion = "0001-production-platform";
