...
import type {
  AgentMode, AgentOutcomeV1, AgentRunDetail, AgentRunState, AgentRunV1, AgentSettingsV1,
...
function RunDrawer({ run, onClose }: { run: AgentRunDetail; onClose: () => void }) {
  ...
  const nextOutcomeCheck = !run.outcome || run.outcome.status !== "PENDING" ? null
    : run.outcome.nextOpenPriceMicros === null
      ? `Next-open check · ${shortTime(run.outcome.observationDueAt)}`
      : run.outcome.plus60mPriceMicros === null
        ? `+60-minute check · ${shortTime(new Date(new Date(run.outcome.observationDueAt).getTime() + 60 * 60_000).toISOString())}`
        : "Cash-close check queued";
  return <motion.div className="agent-overlay drawer" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <motion.aside ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="run-detail-title" className="agent-run-drawer" initial={{ x: 40 }} animate={{ x: 0 }} exit={{ x: 40 }}>
      ...
      <section className="agent-detail-section"><h3>Transition timeline</h3><ol className="agent-trace">{run.transitions.map((item) => <li key={item.id}><i/><span><strong>{stateLabel(item.toState)}</strong><small>{item.reasonCode} · {shortTime(item.createdAt)}</small></span></li>)}</ol></section>
      {run.outcome && <section className="agent-detail-section"><h3>Outcome monitoring</h3><div className="agent-monitoring-card"><Clock3/><span><small>{stateLabel(run.outcome.status)}</small><strong>{nextOutcomeCheck ?? `Scored · ${shortTime(run.outcome.scoredAt)}`}</strong></span></div><p>{run.outcome.status === "PENDING"
        ? `Monitoring was created ${shortTime(monitoringScheduledAt)}. That timeline time is when the schedule was registered—not the observation deadline.`
        : run.outcome.label}</p></section>}
    </motion.aside>
  </motion.div>;
}
  ...
  return <motion.div className="agent-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
    ...
    <main className="agent-shell">
      ...
      <section className="agent-controls"><div className="agent-section-title"><span className="agent-kicker">HUMAN CONTROL PLANE</span><h2>Choose how far the loop may go.</h2><p>Every level runs the same analyst and deterministic guard. Only one level can produce a Demo capability.</p></div><div className="agent-mode-grid">{([
        ...
        {settings && <div className="agent-policy-card"><div><span className="agent-kicker">TIGHTEN-ONLY SETTINGS</span><h3>Monitored rTokens</h3><div className="agent-symbols">{symbols.map((symbol) => <button aria-pressed={settings.symbols.includes(symbol)} className={settings.symbols.includes(symbol) ? "selected" : ""} onClick={() => toggleSymbol(symbol)} key={symbol}><span>{symbolMetadata[symbol].underlyingSymbol.slice(0, 2)}</span><strong>{symbolMetadata[symbol].displaySymbol}</strong><small>{settings.symbols.includes(symbol) ? "MONITORED" : "OFF"}</small></button>)}</div></div><div className="agent-sliders"><label><span>Off-hours trigger <strong>{settings.offHoursMoveThresholdBps} bps</strong></span><input type="range" min="10" max="100" step="5" value={settings.offHoursMoveThresholdBps} onChange={(event) => setSettings({ ...settings, offHoursMoveThresholdBps: Number(event.target.value) })}/></label><label><span>Auto order maximum <strong>{money(settings.automaticOrderLimitCents)}</strong></span><input type="range" min="100" max="10000" step="100" value={settings.automaticOrderLimitCents} onChange={(event) => setSettings({ ...settings, automaticOrderLimitCents: Number(event.target.value) })}/></label><label className="agent-check"><input type="checkbox" checked={settings.notificationsEnabled} onChange={(event) => setSettings({ ...settings, notificationsEnabled: event.target.checked })}/><span>Deliver agent alerts to verified channels</span></label><button className="button button-ink" disabled={busy || !user} onClick={() => void act(() => save(), "Agent settings saved.")}><ShieldCheck/> Save constraints</button></div></div>}
      </section>
